#!/usr/bin/env python3
"""
VoxNova Studio — Offline Text-to-Speech desktop app using pyttsx3 + ttkbootstrap.

Save this file as VoxNova_Studio.py and run:
    python VoxNova_Studio.py
"""
...
class VoxNovaApp:
    ...
    def _apply_initial_geometry(self, w: int, h: int, x: Optional[int], y: Optional[int]):
        try:
    ...
    def _build_ui(self):
        ...
        self.rate_scale = ttk.Scale(rate_row, from_=80, to=220, orient=tk.HORIZONTAL, command=lambda v: self._on_rate_change())
        ...
        self.volume_scale = ttk.Scale(vol_row, from_=0, to=100, orient=tk.HORIZONTAL, command=lambda v: self._on_volume_change())
    ...
    def on_exit(self):
        """Graceful shutdown and config save."""
        try:
            if self.is_busy:
                try:
                    self.stop_now()
        ...
        # Cleanup ttkbootstrap style instance (if any)
        try:
            TBStyle.instance = None
        except Exception:
            pass
    ...
    def toggle_theme(self):
        ...
        try:
            ...
            pass
        # Update the theme toggle label to indicate the action (i.e. click to switch)
